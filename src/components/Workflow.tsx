...
export default function WorkFlow() {
  ...
  return (
    <section style={{ marginTop: "-30vh"}} ref={scrollRef} className="w-full font-sans bg-white py-32 px-8">
      ...
      {/* 🌈 Timeline */}
      <div className="flex w-full items-start gap-12 pt-[0vh]">
        {/* 🌀 Sticky Animated Timeline Label */}
        <motion.h2
          style={{ x: translateX }}
          className="text-6xl font-bold uppercase text-purple-700 sticky top-2/3 -rotate-90 origin-left"
        >
          Timeline
        </motion.h2>
          ...
          {steps.map((step, i) => {
            ...
            return (
              <motion.li
                key={i}
                ref={ref}
                className={`text-8xl font-bold uppercase transition duration-800 ${
                  inView
